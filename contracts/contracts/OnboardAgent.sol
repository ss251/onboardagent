...
contract OnboardAgent {
    ...
    function handleIntent(string memory intent, string memory data) public {
        if (compareStrings(intent, "cast_to_farcaster")) {
            ...
            runAgent(intent, data, 5);
        } else if (compareStrings(intent, "view_wallet_info")) {
            runAgent(intent, data, 5);
        } else {
    ...
    function modifySystemPrompt(string memory intent) private {
        ...
        } if (compareStrings(intent, "view_wallet_info")) {
            prompt = "You are a helpful assistant that analyzes wallet information. Your task is to provide insights and summaries based on the wallet data provided. Focus on key aspects such as token balances, NFT holdings, and recent transactions. Offer clear and concise information that would be valuable to the wallet owner.";
        } else {
